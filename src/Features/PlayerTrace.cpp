...
void PlayerTrace::AddPoint(size_t trace_idx, void *player)
{
    ...
    if (update) {
        trace.updates.push_back({
            ...
            (int32_t) pos.x * TRACE_SCALE_UPDATE,
            (int32_t) pos.y * TRACE_SCALE_UPDATE,
            (int32_t) pos.z * TRACE_SCALE_UPDATE,
            grounded
        });
        delta = {0, 0, 0};
    }
        ...
        for (size_t i = 1; i < trace.deltas.size(); i++) {
            ...
            if (update_idx < trace.updates.size() && i == trace.updates[update_idx].tick) {
                ...
                is_grounded = trace.updates[update_idx].grounded 
                            && !(update_idx < trace.updates.size() && trace.updates[update_idx+1].tick == i+1);
