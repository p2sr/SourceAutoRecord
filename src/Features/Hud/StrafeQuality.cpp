...
void StrafeQualityHud::Paint(int slot)
{
    ...
    for (int i = 0; i < ticks.size(); ++i) {
        ...
        if (i >= ticksAvg - 1) {
            Color bg =
                tick.jumped ? Color{ 0, 80, 120, 255 } :
                tick.grounded && !tick.landed ? Color{ 120, 40, 40, 255 } :
                Color{ 50, 120, 50, 255 };
...
void StrafeQualityHud::OnMovement(int slot, bool grounded)
{
    ...
    StrafeDir strafe =
        (cmd.buttons & IN_MOVELEFT) && !(cmd.buttons & IN_MOVERIGHT) ? StrafeDir::LEFT :
        (cmd.buttons & IN_MOVERIGHT) && !(cmd.buttons & IN_MOVELEFT) ? StrafeDir::RIGHT :
        StrafeDir::NONE;
