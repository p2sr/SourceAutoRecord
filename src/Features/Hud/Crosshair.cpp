...
{
    if (!sar_crosshair_mode.GetBool() && !sar_quickhud_mode.GetBool()) {
        return;
    }
    ...
    } else if (sar_quickhud_mode.GetInt() == 2 && this->isCustomQuickHudReady && portalGunUpgradeState) { // Quickhud from .png
