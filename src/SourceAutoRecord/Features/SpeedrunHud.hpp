...
void Draw()
{
    ...
    auto total = Speedrun::SAR_Timer->GetTotal();
    auto ipt = Speedrun::SAR_Timer->GetIntervalPerTick();

    Surface::Draw(Scheme::GetDefaultFont(), 0, 0, Color(255, 255, 255, 255), "%s", Speedrun::TimerFormat(total * ipt).c_str());
