...
int GetFontLength(HFont font, char* fmt, ...)
{
    va_list argptr;
    va_start(argptr, fmt);
    char data[1024];
...
void Draw(HFont font, int x, int y, Color clr, char* fmt, ...)
{
    va_list argptr;
    va_start(argptr, fmt);
    char data[1024];
...
void DrawRect(Color clr, int x0, int y0, int x1, int y1, HFont font, Color fontClr, char* fmt, ...)
{
