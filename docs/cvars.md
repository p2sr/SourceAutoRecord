...
|ghost_color_b|255|Blue component of ghost color (linear RGB).<br>|
|ghost_color_g|255|Green component of ghost color (linear RGB).<br>|
|ghost_color_r|255|Red component of ghost color (linear RGB).<br>|
...
|ghost_type|cmd|ghost_type \<0/1/2/3>:<br>0: Colored circle<br>1: Colored pyramid<br>2: Colored pyramid with portal gun (RECORDED IN DEMOS)<br>3: Prop model (RECORDED IN DEMOS)<br>|
...
|sar_aim_point_clear|cmd|sar_aim_point_cleat - clear all frozen aimpoints<br>|
...
|sar_con_filter_allow|cmd|sar_con_filter_allow \<string> - add an allow rule to the console filter<br>|
|sar_con_filter_block|cmd|sar_con_filter_block \<string> - add a disallow rule to the console filter<br>|
...
|sar_find_client_class|cmd|sar_find_clientclass \<class_name> - finds specific client class tables and props with their offset<br>|
|sar_find_client_offset|cmd|sar_find_client_offset \<class_name> \<prop_name> - finds prop offset in specified client class<br>|
...
|sar_find_server_offset|cmd|sar_find_server_offset \<class_name> \<prop_name> - finds prop offset in specified server class<br>|
...
|sar_function|cmd|sar_function \<name> [command] [args]... - create a function, replacing $1, $2 etc up to $9 in the command string with the respective argument. If no command is specified, prints the given function<br>|
...
|sar_hud_order_reset|cmd|sar_hud_order_reset - resets order of hud element<br>|
...
|sar_hud_set_text_color|cmd|sar_hud_set_text_color \<id> \<color> - sets the color of the nth text value in the HUD<br>|
...
|sar_hud_velocity|0|Draws velocity of the client.<br>0 = Default,<br>1 = X, Y, Z<br>2 = X:Y<br>3 = X:Y, Z<br>4 = X:Y:Z<br>|
...
|sar_ihud|0|Enabled or disables movement inputs HUD of client.<br>|
...
|sar_ihud_modify|cmd|sar_ihud_modify \<element|all> [param=value]... - modifies parameters in given element.<br>Params: enabled, text, font, pos, x, y, width, height, font, background, highlight, textcolor, texthighlight.<br>|
|sar_ihud_preset|cmd|sar_ihud_preset \<preset> - modifies input hud based on given preset<br>|
|sar_ihud_setpos|cmd|Automatically sets the position of input HUD.<br>Usage: sar_ihud_setpos \<top|center|bottom|y|y%> \<left|center|right|x|x%><br>|
...
|sar_loads_norender|0|Temporarily set mat_noredering to 1 during loads<br>|
...
|sar_lphud_setpos|cmd|Automatically sets the position of least portals HUD.<br>Usage: sar_lphud_setpos \<top|center|bottom|y|y%> \<left|center|right|x|x%><br>|
...
|sar_player_trace_autoclear|1|Automatically clear the trace on session start<br>|
...
|sar_player_trace_clear|cmd|sar_player_trace_clear \<index> - Clear the index player trace<br>|
|sar_player_trace_clear_all|cmd|sar_player_trace_clear_all - Clear all the traces<br>|
|sar_player_trace_draw|0|Display the recorded player trace. Requires cheats<br>|
|sar_player_trace_draw_speed_deltas|1|Display the speed deltas. Requires sar_player_trace_draw<br>|
|sar_player_trace_draw_through_walls|1|Display the player trace through walls. Requires sar_player_trace_draw<br>|
|sar_player_trace_draw_time|3|Display tick above trace hover info<br>0 = hide tick info<br>1 = ticks since trace recording started<br>2 = session timer<br>3 = TAS timer (if no TAS was played, uses 1 instead)<br>|
|sar_player_trace_record|0|Record the trace to a slot. Set to 0 for not recording<br>|
|sar_player_trace_teleport_at|cmd|sar_player_trace_teleport_at \<tick> [player slot] [trace index] - teleports the player at the given trace tick on the given trace ID (defaults to 1) in the given slot (defaults to 0).<br>|
...
|sar_render_shutter_angle|180|The shutter angle to use for rendering in degrees.<br>|
...
|sar_speedrun_cc_rule|cmd|sar_speedrun_rule \<rule name> \<rule type> [options]... - add a rule to the category currently being created<br>|
...
|sar_speedrun_offset|0|Start speedruns with this many ticks on the timer.<br>|
...
|sar_speedrun_stop|cmd|sar_speedrun_start - stop the speedrun timer<br>|
...
|sar_tas_autosave_raw|0|Enables automatic saving of raw, processed TAS scripts.<br>|
...
|sar_teleport|cmd|sar_teleport - teleports the player to the last saved location<br>|
...
|sar_trace_a|cmd|sar_trace_a - saves location A for tracing<br>|
|sar_trace_b|cmd|sar_trace_b - saves location B for tracing<br>|
|sar_trace_reset|cmd|sar_trace_reset - resets tracer<br>|
|sar_trace_result|cmd|sar_trace_result - prints tracing result<br>|
...
|svar_from_cvar|cmd|svar_from_cvar \<variable> \<cvar> - capture a cvars's value and place it into an svar, removing newlines<br>|
